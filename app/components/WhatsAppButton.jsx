...
const WhatsAppButton = ({ number, message }) => {
    ...
    // Cleanup number (remove spaces, dashes, parentheses)
    // Ensure it has no special chars for the URL
    const cleanNumber = number.replace(/[^\d+]/g, '');

    // If number doesn't start with +, usually it's better to verify logic. 
    // But generally wa.me works with country code. Assuming user enters it.
    ...
    return (
        ...
        >
            <svg
                ...
                viewBox="0 0 24 24"
                fill="white"
                className="fill-current text-white transform group-hover:rotate-12 transition-transform duration-300"
            >
