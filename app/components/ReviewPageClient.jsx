...
import PlatformCard, { PlatformIcon } from "./PlatformCard";
import { siteConfig } from "../data/siteConfig";
import { staticReviewsByPlatform } from "../data/reviewsData";

export default function ReviewPageClient({ initialProfile, isNotFound }) {
    ...
    useEffect(() => {
        ...
        if (initialProfile) {
            ...
            initializePlatforms(initialProfile);
        } else {
            // No profile provided (no slug or not found but not explicitly 404'd by server logic yet, though server should handle that)
            // If we are here, it means we want static config
            loadStaticConfig();
        }
    }, [initialProfile, isNotFound]);
    ...
    const initializePlatforms = (profileData) => {
        // Strictly use DB platforms. If null/undefined, assume empty
    ...
    // CHECK FOR SUSPENDED/INACTIVE ACCOUNT
    if (currentProfile && currentProfile.isActive === false) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4 text-center">
                <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-gray-100">
                    ...
                    <p className="text-gray-600 mb-6">
                        This business profile is currently inactive. Please contact the administrator for assistance.
                    </p>
